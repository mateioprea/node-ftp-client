...
Client.prototype.download = function (source, dest, options, downloadCallback) {
    ...
    var queue = async.queue(function (task, callback) {
        console.log('Queue worker started for ' + task.src, 'debug');
            ...
            ftp.get(file, function (err, stream) {
                ...
                if (stream) {
					...
					stream.on('data', function(chunk) {
						index = chunk.toString().indexOf('\n');
						acc += chunk;
						console.log(index);
						index !== -1 ? stream.destroy() : pos += chunk.length;
					}).on('close', function() {
						result.header = acc.slice(0, pos + index)
						callback()
					});
                }
            });
